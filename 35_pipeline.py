...
def pipeline_model(frame):
    ...
    if len(detections) > 0:
        for i, confidence in enumerate(detections[0, 0, :, 2]):
            if confidence > 0.5:
                ...
                cv2.putText(frame, text_face, (startx, starty), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)
    ...
    return frame, machinelearning_results
...
while True:
    ...
    frame, results = pipeline_model(frame)
    time.sleep(0.5)
